...
public class LocalTimedCacheHolder<K, V> implements CacheHolder<K, V> {
     ...
     */
    public LocalTimedCacheHolder(ApiConfig config) {
        this.cache = new TimedCache<>(TimeUnit.SECONDS.toMillis(config.getTokenExpireSeconds()) - DEFAULT_EXPIRE_BUFFER);
        this.config = config;
    }
    ...
    public void put(K key, V object) {
        cache.put(key, object, TimeUnit.SECONDS.toMillis(config.getTokenExpireSeconds()) - DEFAULT_EXPIRE_BUFFER);
    }
}
